...
# =======================
# NEW: SECURE CODE GENERATION SYSTEM (ADDED WITHOUT REMOVING ANYTHING)
# =======================

SECURE_CODING_PROMPT_PREFIX = """
CRITICAL SECURITY MANDATE: You must generate ONLY secure, production-ready code that follows these NON-NEGOTIABLE requirements:

MANDATORY SECURITY RULES:
1. NEVER use eval(), exec(), compile() - use safe alternatives only
2. NEVER use subprocess without shell=False and proper input validation
3. NEVER use pickle/marshal - use JSON for serialization
4. SQL queries MUST use parameterized statements with ? placeholders
5. File operations MUST validate paths and use allowlists
6. Input validation MUST be implemented on ALL user inputs
7. Use try-except blocks with specific exception handling
8. Implement proper authentication and authorization checks
9. Sanitize ALL outputs to prevent injection attacks
10. Follow principle of least privilege throughout
11. Add security comments explaining protections
12. Use secure defaults and fail-safe mechanisms

USER REQUEST: {user_request}

Generate ONLY secure, production-ready implementation:"""


def make_prompt_secure(original_prompt):
    """Convert code generation prompts to secure prompts, but preserve analysis questions"""

    # First check if this is an analysis/question about existing code
    analysis_patterns = [
        'is this code secure', 'is the code secure', 'analyze this code','is the code secure?'
        'check this code', 'review this code', 'secure this code',
        'vulnerabilities in', 'security issues', 'is this safe'
    ]

    prompt_lower = original_prompt.lower()

    # If it's a security analysis question, don't convert it
    if any(pattern in prompt_lower for pattern in analysis_patterns):
        return original_prompt

    # Only convert actual code generation requests
    generation_keywords = ['create', 'build', 'write', 'develop', 'generate', 'make', 'implement']
    code_targets = ['function', 'class', 'script', 'program', 'code for', 'code that']

    is_generation_request = any(
        gen_word in prompt_lower and any(target in prompt_lower for target in code_targets)
        for gen_word in generation_keywords
    )

    if is_generation_request:
        return SECURE_CODING_PROMPT_PREFIX.format(user_request=original_prompt)

    return original_prompt


def is_educational_code(code_content):
    """Detect basic educational code that should have reduced security scanning"""
    educational_indicators = [
        'fibonacci', 'factorial', 'prime', 'hello world', 'basic loop',
        'simple calculation', 'sorting algorithm', 'array example',
        'string manipulation', 'number pattern', 'mathematical sequence'
    ]

    dangerous_indicators = [
        'subprocess', 'os.system', 'eval', 'exec', 'pickle', 'marshal',
        'shell=True', '__import__', 'compile', 'input()', 'raw_input'
    ]

    code_lower = code_content.lower()

    # Check for educational patterns
    is_educational = any(indicator in code_lower for indicator in educational_indicators)

    # Check for dangerous patterns
    has_dangerous = any(indicator in code_lower for indicator in dangerous_indicators)

    # Check code complexity (educational code is usually simple)
    line_count = len(code_content.split('\n'))
    is_simple = line_count < 50

    return is_educational and not has_dangerous and is_simple


# =======================
# ORIGINAL DEMO MODE (PRESERVED)
# =======================
...
# =======================
# ENHANCED MESSAGE PROCESSING (PRESERVES ALL ORIGINAL FEATURES + ADDS SECURE GENERATION)
# =======================

def safe_process_secure_message(user_input):
    """Safely process message with ENHANCED SECURE CODE GENERATION + ALL ORIGINAL FEATURES"""
    ...
    try:
        ...
        with st.spinner("Running security analysis..."):
            ...
            try:
                ...
                # NEW: STEP 1 - CONVERT TO SECURE PROMPT AUTOMATICALLY
                secure_prompt = make_prompt_secure(safe_prompt)
                if secure_prompt != safe_prompt:
                    st.info("🔒 Code request detected - applying secure generation template")

                # Generate response with context
                if st.session_state.full_context:
                    context_messages = st.session_state.full_context[-10:]
                    llm_response = llm_client.generate_with_context(secure_prompt, context_messages)
                else:
                    llm_response = llm_client.generate(secure_prompt)

                # ANALYZE THE GENERATED RESPONSE FOR CODE VULNERABILITIES (ORIGINAL FEATURE PRESERVED)
                code_analysis = None
                if any(marker in llm_response for marker in
                       ['```', 'def ', 'class ', 'import ', 'subprocess', 'os.system', 'pickle', 'eval', 'exec']):
                    try:
                        # NEW: Check if it's educational code first
                        if is_educational_code(llm_response):
                            st.info("📚 Educational code detected - applying reduced security scanning")
                            analysis_results['educational_code'] = True
                            # Reduce risk score for educational code
                            analysis_results['risk_score'] = min(analysis_results['risk_score'], 25)
                        else:
                            # NEW: PROACTIVE SECURITY ENHANCEMENT FOR NON-EDUCATIONAL CODE
                            st.info("⚡ Code detected - applying proactive security enhancements...")

                            enhancement_prompt = f"""
CRITICAL SECURITY ENHANCEMENT: The following code must be made completely secure for production:

{llm_response}

MANDATORY REQUIREMENTS:
1. Add comprehensive input validation for ALL parameters
2. Implement proper error handling with safe error messages
3. Add authentication/authorization checks where applicable
4. Use secure defaults and fail-safe mechanisms
5. Add security comments explaining protections
6. Ensure no hardcoded credentials or sensitive data
7. Implement rate limiting where needed
8. Add security logging (without exposing sensitive data)
9. Use parameterized queries for database operations
10. Sanitize all outputs to prevent injection attacks

Provide ONLY the enhanced secure version:"""

                            try:
                                enhanced_response = llm_client.generate(enhancement_prompt)
                                llm_response = enhanced_response
                                st.success("✅ Code automatically enhanced with security measures")
                                analysis_results['security_enhanced'] = True
                            except Exception as e:
                                logger.warning(f"Security enhancement failed: {e}")

                        # ORIGINAL CODE ANALYSIS (PRESERVED)
                        ...
                        # For educational code, cap the risk score
                        if analysis_results.get('educational_code', False):
                            combined_risk = min(max(original_risk, code_risk), 30)
                        else:
                        ...
                        # ORIGINAL SANDBOX RUNTIME ANALYSIS (PRESERVED)
                        try:
                            ...
                            if sandbox_result['suspicious_behavior']:
                                ...
                                if not analysis_results.get('educational_code', False):
                        ...
                        if code_risk > 70 and not analysis_results.get('educational_code', False):
                        ...
                        # ORIGINAL ITERATIVE SECURITY REFINEMENT (PRESERVED + ENHANCED)
                        vulnerabilities = code_analysis.get('vulnerabilities', [])
                        if vulnerabilities and not analysis_results.get('educational_code', False):
                            max_iterations = 3
                            current_response = llm_response
                            iteration = 0
                            initial_vulns = len(vulnerabilities)

                            st.info(
                                f"🚨 Found {initial_vulns} security vulnerabilities. Starting mandatory refinement...")
...
Please rewrite the code to be completely secure while preserving all original functionality. Provide ONLY the corrected, secure code with security improvements."""
            ...
            # ORIGINAL PERFORMANCE MONITORING (PRESERVED)
            try:
...
# =======================
# ALL ORIGINAL UI FUNCTIONS (PRESERVED EXACTLY)
# =======================
    ...
    with st.sidebar:
        ...
        if st.session_state.analysis_results:
            ...
            if st.session_state.analysis_results and isinstance(st.session_state.analysis_results, dict):
                ...
                # NEW: Show new security features
                if results.get('security_enhanced'):
                    improvements.append("Proactive security enhancement")
                if results.get('educational_code'):
                    improvements.append("Educational code detected")
    ...
    if not st.session_state.conversation_history:
        with st.chat_message("assistant"):
            ...
            **Enhanced Security Features:**
            ...
            - **NEW: Proactive secure code generation**
            - **NEW: Educational code recognition**
...
def process_with_iterative_security(prompt, llm_client, prompt_id, protected_context=None, max_iterations=3):
    ...
    contains_code = any(marker in llm_response for marker in
                        ['```', 'def ', 'class ', 'import ', 'SELECT', 'INSERT', 'CREATE', 'UPDATE', 'DELETE'])
    ...
    if contains_code:
        ...
        while iteration < max_iterations:
            ...
            if iteration == max_iterations - 1:
                st.warning(
                    f"Reached maximum iterations ({max_iterations}). Code still contains {len(code_analysis['vulnerabilities'])} vulnerabilities.")
                break
...
Please rewrite the code to be completely secure while maintaining the same functionality. Provide ONLY the corrected code with security improvements."""
            ...
            st.info(
                f"Iteration {iteration + 1}: Found {len(code_analysis['vulnerabilities'])} vulnerabilities. Refining code...")
    ...
    if contains_code:
        ...
        if analysis_results['final_vulnerabilities'] < original_vulns:
            ...
            st.success(
                f"Security improved: {original_vulns} → {analysis_results['final_vulnerabilities']} vulnerabilities")
...
def render_logs_page():
    ...
    try:
        ...
        if logs:
            ...
            with col3:
                action_filter = st.selectbox("Action Type",
                                             ["All Actions", "Prompt Analysis", "Response Generation",
                                              "Security Alert"])
        ...
        sample_logs = [
            {"timestamp": "2024-01-15 14:30", "action": "Prompt analyzed", "risk_score": 25,
             "details": "Standard prompt analysis completed"},
            {"timestamp": "2024-01-15 14:28", "action": "High-risk prompt detected", "risk_score": 85,
             "details": "Potential injection attempt blocked"},
            {"timestamp": "2024-01-15 14:25", "action": "Response sanitized", "risk_score": 65,
             "details": "Code vulnerabilities removed from response"},
            {"timestamp": "2024-01-15 14:20", "action": "Code vulnerability found", "risk_score": 75,
             "details": "SQL injection pattern detected in generated code"},
            {"timestamp": "2024-01-15 14:15", "action": "Normal interaction", "risk_score": 15,
             "details": "Routine conversation processed"}
        ]
...
def render_settings_page():
    ...
    try:
        ...
        current_settings = {
            ...
            'enable_logging': True,
            'retention_days': 30,
            'force_secure_code': True,
            'educational_code_detection': True
        }

    with st.form("settings_form"):
        st.markdown("### Enhanced Security Settings")

        force_secure_code = st.checkbox("Force Secure Code Generation",
                                        value=current_settings.get('force_secure_code', True),
                                        help="Automatically convert code requests to secure generation prompts")

        educational_code_detection = st.checkbox("Educational Code Detection",
                                                 value=current_settings.get('educational_code_detection', True),
                                                 help="Reduce security scanning for basic educational examples like Fibonacci")
        ...
        if save_settings:
            new_settings = {
                ...
                'force_secure_code': force_secure_code,
                'educational_code_detection': educational_code_detection
            }
    ...
    with col1:
        ...
        if force_secure_code:
            st.success("Secure Code Generation: Enabled")
        else:
            st.warning("Secure Code Generation: Disabled")

        if educational_code_detection:
            st.success("Educational Code Detection: Enabled")
        else:
            st.warning("Educational Code Detection: Disabled")
